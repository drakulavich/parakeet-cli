...
**English acronym handling** (`en-*` Kokoro-82M only): all-uppercase Latin tokens (length 2–5) auto-expand letter-by-letter — `EPAM` → `ee-pee-ay-em`, `FBI` → `ef-bee-eye`. A 30-entry stop-list passes natural-English caps words (NASA, NATO, AIDS, OPEC, IKEA, ASCII, NAFTA, LASER, RADAR, SCUBA) and emphatic short caps that are real words (OK, NO, GO, IT, IS, AS, AT, BY, IN, ON, OR, OF, TO, WE, US, MY, ME, HE, BE, DO) through unchanged. Disable per call with `--no-expand-abbrev`. Override per-token via SSML `<say-as interpret-as="characters">…</say-as>` (always wins). Closes [#244](https://github.com/drakulavich/kesha-voice-kit/issues/244).
